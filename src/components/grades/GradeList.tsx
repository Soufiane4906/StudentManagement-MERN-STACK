...
export function GradeList() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedGrade) {
        await api.patch(`/grades/${selectedGrade.id}`, formData);
      } else {
            ...
            {grades.map((grade) => (
              <tr key={grade.id}>
                <td className="px-6 py-4 whitespace-nowrap">
                  {students.find(s => s.id === grade.student_id)?.first_name} {students.find(s => s.id === grade.student_id)?.last_name}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  {courses.find(c => c.id === grade.course_id)?.name}
                </td>
